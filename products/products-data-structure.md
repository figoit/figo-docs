...
- **`brand`**: Embedded brand/manufacturer information
...
- **`categories`**: Embedded category hierarchy with flexible levels
...
db.products.createIndex({ "categories.lastCategory._id": 1 })
db.products.createIndex({ "categories.lastCategory.departmentId": 1 })
db.products.createIndex({ "categories.lastCategory.parentId": 1 })
...
db.products.find({
  "categories.hierarchy._id": ObjectId("cat4")
})

// Find all products in "Moda" department (level 1)
db.products.find({
  "categories.lastCategory.departmentId": ObjectId("cat1")
})
```
...
db.products.find({
  "categories.lastCategory._id": ObjectId("cat5")
})

// With aggregation for breadcrumb
db.products.aggregate([
  { $match: { "categories.lastCategory._id": ObjectId("cat5") }},
  { $project: {
    name: 1,
    breadcrumb: "$categories.hierarchy.name"
  }}
])
...
db.products.find({
  "categories.lastCategory.parentId": ObjectId("cat4")
})
```
...
db.products.find({
  "categories.lastCategory.level": 3
})

// Find deep categorized products (level >= 4)
db.products.find({
  "categories.lastCategory.level": { $gte: 4 }
})
```
...
async function getCategoryTree(departmentId) {
  return await db.products.aggregate([
    ...
    { $group: {
      _id: {
        categoryId: "$categories.hierarchy._id",
        level: "$categories.hierarchy.level"
      },
      name: { $first: "$categories.hierarchy.name" },
      productCount: { $sum: 1 }
    }},
